...
$(document).ready(function () {
    ...
    function build() {
        ...
        for (var i = 0; i < timeArray.length; i++) {
            ...
            if (timeArray[i] === 12 || timeArray[i] < 8) {
                ...
                startHTML += optionVal + timeArray[i] + 'PM">' + timeArray[i] + ': 00 PM' + '</option>';
                startHTML += optionVal + timeArray[i] + '30PM">' + timeArray[i] + ': 30 PM' + '</option>';

                endHTML += optionVal + timeArray[i] + 'PM">' + timeArray[i] + ': 00 PM' + '</option>';
                endHTML += optionVal + timeArray[i] + '30PM">' + timeArray[i] + ': 30 PM' + '</option>';
                //give value attributes
                //generate time values to select from

                section += newRow + timeArray[i] + 'PM' + '">';
                section += timeStamp + timeArray[i] + ': 00 PM ' + '</td>';
                section += data + '&nbsp;</td>' + dataDotted + '&nbsp;</td>';
                section += '</tr>';

                section += newRow + timeArray[i] + '30PM' + '">';
                section += minute30 + timeArray[i] + ': 30 PM ' + '</td>';
                section += data30 + '&nbsp;</td>' + data30Dotted + '&nbsp;</td>';
                section += '</tr>';
                ...
                var optionVal = '<option value="';
                startHTML += optionVal + timeArray[i] + 'AM">' + timeArray[i] + ': 00 AM' + '</option>';
                startHTML += optionVal + timeArray[i] + '30AM">' + timeArray[i] + ': 30 AM' + '</option>';

                endHTML += optionVal + timeArray[i] + 'AM">' + timeArray[i] + ': 00 AM' + '</option>';
                endHTML += optionVal + timeArray[i] + '30AM">' + timeArray[i] + ': 30 AM' + '</option>';
                //generate time values to select from

                section += newRow + timeArray[i] + 'AM' + '">';
                section += timeStamp + timeArray[i] + ': 00 AM ' + '</td>';
                section += data + '&nbsp;</td>' + dataDotted + '&nbsp;</td>';
                section += '</tr>';

                section += newRow + timeArray[i] + '30AM' + '">';
                section += minute30 + timeArray[i] + ': 30 AM ' + '</td>';
                section += data30 + '&nbsp;</td>' + data30Dotted + '&nbsp;</td>';
                section += '</tr>';
    ...
    $('#submit').on('click', function () {
        ...
        for (var i = 0; i < rowArray.length; i++) {
            ...
            if (inUseArray[i] === true) {
                alert('Error. Time overlap.');
                break;
            }
                if (i === 0 && isFirst) {
